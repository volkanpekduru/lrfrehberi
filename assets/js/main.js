...
document.addEventListener("DOMContentLoaded", () => {
    ...
    faqTriggers.forEach(trigger => {
        trigger.addEventListener('click', function() {
            ...
            document.querySelectorAll('.lrf-faq__item').forEach(item => item.classList.remove('active'));
    ...
    lightboxTriggers.forEach(trigger => {
        trigger.addEventListener('click', function(e) {
            ...
            const title = this.getAttribute('data-lightbox-title') || this.textContent;
            const isJig = ['round','dart','standup','football','shaky','swimbait','cheburashka','hirsiz'].some(k => imgName.toLowerCase().includes(k));
    ...
    const tabData = {
        ...
        surface: "Tamamen su yüzeyinde çalışan çılgın maketler. Sabah erken ve akşam üstü, balıkların su üstüne oynak yaptığı anlarda katliam yaratır."
    };
    ...
    tabButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            tabButtons.forEach(btn => btn.classList.remove('lrf-sahte-tab-btn--active'));
            ...
            document.querySelectorAll('.lrf-sahte-group').forEach(g => g.classList.remove('lrf-sahte-group--active'));
    ...
    const akordiyonButonlari = document.querySelectorAll('.lrf-grid-card__btn, .lrf-jig-card__btn, .lrf-jig-card__trigger, .lrf-fish-card__btn, .lrf-fish-card__trigger');
    ...
    akordiyonButonlari.forEach(button => {
        button.addEventListener('click', function() {
            ...
            const detayPaneli = cardGövdesi.querySelector('.lrf-jig-card__panel, .lrf-fish-card__panel');
            ...
            if (detayPaneli.classList.contains('lrf-fish-card__panel') && !detayPaneli.classList.contains('active')) {
                document.querySelectorAll('.lrf-fish-card__panel.active').forEach(p => {
                    p.classList.remove('active');
                    const otherCard = p.closest('.lrf-grid-card');
                    const otherBtn = otherCard?.querySelector('.lrf-fish-card__btn, .lrf-fish-card__trigger');
                    if (otherBtn) {
            ...
            if (!this.hasAttribute('data-original-text')) {
                this.setAttribute('data-original-text', butonYazısı ? butonYazısı.textContent : 'Detaylar');
            }
            const orijinalMetin = this.getAttribute('data-original-text');
            const isAçik = detayPaneli.classList.toggle('active');
            this.classList.toggle('active');

            if (butonYazısı) butonYazısı.textContent = isAçik ? 'Gizle' : orijinalMetin;
        });
    });
});
